...
. クラスタ名を指定して bundle を生成します (ここでは ``demo-cluster``と指定 )。
+
...
. リポジトリで提供されているサンプルを使用して、Secured Clusterカスタムリソースを作成します(オプションでカスタマイズも可能)。
+
[.console-input]
[source,bash,subs="attributes+,+macros"]	
----	
...
cd
git clone https://github.com/RH-OPEN/rhacs-demo
cd rhacs-demo
----
...
. Playbook の実行後。すべてが正常に機能した場合出力は次のようになります。
+
[.console-output]
[source,bash,subs="attributes+,+macros"]	
----	
...
もし本ラボの残りのセクションに従う予定がなく、独自の発見をしたい場合は以下の手順を実施してみてください。
